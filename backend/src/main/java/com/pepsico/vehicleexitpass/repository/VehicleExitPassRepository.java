...
public interface VehicleExitPassRepository extends JpaRepository<VehicleExitPass, String> {
    ...
    List<VehicleExitPass> findByOperador(User operador);
    
    @Query("SELECT p FROM VehicleExitPass p WHERE p.fecha BETWEEN :startDate AND :endDate")
    List<VehicleExitPass> findByDateRange(@Param("startDate") LocalDate startDate, 
    ...
    @Query("SELECT p FROM VehicleExitPass p WHERE " +
           ...
           "LOWER(p.tractorEco) LIKE LOWER(CONCAT('%', :search, '%'))")
